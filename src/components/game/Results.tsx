...
export function Results({ shots, expectedSequence, onClose }: ResultsProps) {
  const correct = shots.filter((shot, index) => shot === expectedSequence[index]).length;
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-[#202123] rounded-2xl p-6 max-w-md w-full shadow-2xl border border-[#343541]">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-[#009B70]">
            Résultats
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="space-y-6">
            ...
            <div className="flex gap-3 text-lg">
              {shots.map((shot, i) => (
                <span
                  key={i}
                  className={`
                    ${shot === expectedSequence[i] ? 'text-green-400' : 'text-red-400'}
          ...
          <div className="text-center pt-4">
            ...
            <p className="text-4xl font-bold text-[#009B70]">
              {score}
            </p>
          </div>
        </div>
