...
export const generateInstruction = (): Instruction => {
  const direction = DIRECTIONS[Math.floor(Math.random() * DIRECTIONS.length)].value;
...
export const formatInstruction = (instruction: Instruction, part?: 'direction' | 'target'): string => {
