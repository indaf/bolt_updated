...
type AnnuaireHistoryProps = {
  results: Array<any>;
};

export function AnnuaireHistory({ results }: AnnuaireHistoryProps) {
  ...
  const filteredHistory = results.filter((entry) => {
    if (
      filters.search &&
      !entry.data.name.toLowerCase().includes(filters.search.toLowerCase()) &&
    ...
    if (
      filters.name != "" &&
      entry.data.name.toLowerCase() != filters.name.toLowerCase()
    )
      return false;
          ...
          <tbody>
            {paginatedHistory.map((entry) => (
              <tr key={entry.id} className="border-b border-[#343541]">
                ...
                <td className="px-4 py-3 text-white">{entry.data.name}</td>
                <td className="px-4 py-3 text-white">
                  {entry.data.exercice_id}
                </td>
              </tr>
            ))}
          </tbody>
