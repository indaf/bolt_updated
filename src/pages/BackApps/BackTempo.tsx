...
import { calculateChartDataForTempo } from "../../helpers/Chart.helper";
import { calculateStatsTempoPerUser } from "../../helpers/Object.helper";
import { TempoGameHistory } from "./components/TempoGameHistory";

type Tab = "stats" | "history";

export function BackTempo() {
    ...
    getGameScores("tempo")
      .then((response: AxiosResponse) => {
        ...
        if (response.data.length > 0) {
          setChartData(calculateChartDataForTempo(response.data));
        }
      })
      .catch((error: any) => {
  ...
  const calculateAverageAccuracy = () => {
    return (
      allResults.map((result: any) => result.score).reduce((a, b) => a + b, 0) /
      allResults.length
    );
  };
  ...
  return (
    <Layout pageTitle="Back Office - Tempo Shooting">
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          <AppHeader
            title="Back Office - Tempo Shooting"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div className="bg-[#202123] rounded-lg p-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm text-gray-400">
                        Précision Moyenne / Partie
                      </p>
                      <p className="text-2xl font-medium text-white">
                        {calculateAverageAccuracy().toFixed(2)}%
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-[#202123] rounded-lg p-6">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {calculateStatsTempoPerUser(allResults).map((user) => (
                        <tr
                          key={user.userId}
          ...
          {activeTab === "history" && <TempoGameHistory results={allResults} />}
        </div>
      </div>
    </Layout>
  );
